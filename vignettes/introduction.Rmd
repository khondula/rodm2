...
A list that matches the variables (i.e. column names) to their units id required. Use make_vars_list(ts_data) to create a list with information about each variable in the dataset to be inserted. 
...
```{r}
varnames <- get_cv_terms("variablename", quietly = TRUE) 
...
```{r}
my_data <- rodm2::db_get_results(db, result_type = "ts")

head(my_data$`Time series data`)
```
